...
public class ForgeFuelManager extends SimpleJsonResourceReloadListener {
    ...
    private static void parseSingleEntry(JsonObject json) {
        ...
        if (json.has("ingredient")) {
            ingredient = Ingredient.CODEC.parse(JsonOps.INSTANCE, json.get("ingredient"))
                    .result().orElse(null);
    ...
    public static FuelInfo getFuelInfo(ItemStack stack) {
        ...
        for (FuelEntry entry : REGISTERED_FUELS) {
            if (entry.ingredient.test(stack)) {
                int burnTime = entry.burnTime > 0 ? entry.burnTime : stack.getBurnTime(RecipeType.BLASTING);
        ...
        if (bestMatch == null) {
            int furnaceBurnTime = stack.getBurnTime(RecipeType.BLASTING);
